...
const HomecoffeeScreen = () => {  
    ...
    const fetchProducts = async () => {
        try {
            const response = await fetch('http:10.24.51.6:3000/produc_bean');
    ...
    const handleSave = async () => {
        ...
        try {
            let response;
            if (editingId) {
                response = await fetch(`http:10.24.51.6:3000/produc_bean/${editingId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(productData)
                });
            } else {
                response = await fetch('http:10.24.51.6:3000/produc_bean', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(productData)
                });
    ...
    const handleDelete = async (id: number) => {
        Alert.alert('Xác nhận', 'Bạn có chắc muốn xóa sản phẩm này?', [
            { text: 'Hủy', style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        await fetch(`http:10.24.51.6:3000/produc_bean/${id}`, { method: 'DELETE' });
                        fetchProducts();
                    } catch (error) {
